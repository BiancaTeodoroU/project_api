async function listUsers() {
    ...
    return fetch("https://agile-stream-76831.herokuapp.com/user/", requestOptions)
    .then((response) => {
...
async function CreateUser(dados) {
    ...
    const request = await fetch("https://agile-stream-76831.herokuapp.com/user/", requestOptions)
    return request
}
...
async function SubmeterDados(event) {
    ...
    if(userId){
        await updateUser(dados, userId).then(async res => {
            ...
            console.log('Sucesso')
            console.log(res)
            document.location.href=`https://agile-stream-76831.herokuapp.com/static/index.html`
            }).catch ( error => {
            console.error(error);
            })
    }
...
async function updateUser(user, id) {
    ...
    const request = await fetch(`https://agile-stream-76831.herokuapp.com/user/${id}`, requestOptions)
    return request 
}
...
async function DeleteUser(user) {
    ...
    const request = await fetch(`https://agile-stream-76831.herokuapp.com/user/${user._id}`, requestOptions);
    return request;
}
